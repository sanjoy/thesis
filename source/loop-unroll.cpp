/*
  ...
  2) if the compiler unrolls the loop, it should not be slower than
  the original loop without unrolling

  3) the compiler should unroll a multi-calculation loop as well as a
  single calculation loop up to the limit of performance gain for
  unrolling that loop in other words: no penalty for manually
  unrolling, as long as the manual unroll is less than or equal to the
  optimum unroll factor

  4) The compiler should recognize and unroll all loop styles with the
  same efficiency in other words: do, while, for, and goto should have
  identical performance

*/

#include "benchmark-stdint.hpp"
#include <stddef.h>
#include <stdio.h>
#include <time.h>
#include <stdlib.h>
...
#include "benchmark-shared-tests.h"
