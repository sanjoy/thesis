/*
  Copyright 2007-2008 Adobe Systems Incorporated Distributed under
  the MIT License (see accompanying file LICENSE_1_0_0.txt or a copy
  at http://stlab.adobe.com/licenses.html ) Source file for tests
  shared among several benchmarks */
...
inline bool tolerance_equal(float &a, float &b) {
  ...
  if (fabs(a) > 1.0e-4)
    reldiff = diff / a;
  // Single precision divide test is really imprecise
...
void test_CSE(T* first, int count, T v1, const char *label) {
  ...
  for (int i = 0; i < iterations; ++i) {
    ...
    result +=
        first[0] + Shifter::do_shift( v1, first[0], first[1] ) +
        Shifter::do_shift( v1, first[0], first[1] );
    result -=
        first[1] + Shifter::do_shift( v1, first[0], first[1] ) +
        Shifter::do_shift( v1, first[0], first[1] );

    for (int n = 1; n < count; ++n) {
      result +=
          first[n-1] + Shifter::do_shift( v1, first[n-1], first[n] ) +
          Shifter::do_shift( v1, first[n-1], first[n] );
      result -=
